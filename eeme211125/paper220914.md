...
It is widely known that improvisation and experimentation are key factors in the development of the arts and sciences. With SuperCollider one can experiment with code scripts that already exist on the internet as well as create one's own sounds in real time (live coding). The adaption of existing code examples and the alternation of their parameters can enrich the musical programming vocabulary. In this way a musical performance can be modified easily and quickly. Modifications are essential to live music performances
