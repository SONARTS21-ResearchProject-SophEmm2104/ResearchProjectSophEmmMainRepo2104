...
We propose the use of SuperCollider for children above the age of ten that they already have some courses of English Language. The use of SuperCollider in music education can be beneficial also for children in order to advance in other subjects such as physics, mathematics, english and coding. SuperColider can be considered as an Interdisciplinary enviroment that combines all the former mentioned sciences together. This is a unique feature that are not offered by the common use of DAWs.
...
One can learn SuperCollider only by try and error process. As it was decribed above, SuperCollider is an open-source IDE which has an evolving community around it. For one that want to learn how to use it is very easy to find tutorials of its use and pre-existing examples of code that correspond to one's needs. When working with children we recomment that this search is made from the music teacher and the experimentation ...
...
The cost of this effort can be measured only in time. It's
