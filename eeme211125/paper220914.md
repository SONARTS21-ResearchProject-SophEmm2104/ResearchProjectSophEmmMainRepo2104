...
Αs aforementioned in previous chapters of this paper (chapters 2,3), with the use of SuperCollider one can synthesize one's sounds from scratch, an opportunity which is not applicable to many DAWs. Αs is the case with other DAWS, with the Supercollider one can also process the audio signal and mix it with the difference of course that in the SuperCollider this processing is done through the use of code. The general use code introduces us to processing analysis mindset and data processing, aspects that are known in computer sciences.

Moreover, a music teacher can use this environment in order to explain the sound parameters to his/her music students e.g. harmonics. Furthermore, in an more advanced level a music teacher can teach students the process of making their own interactive sound systems by connecting their created sounds to sensors and/or other hardware. Needless to say that the involvement with programming can be beneficial for learning other subjects too such as coding, computer sciences, English language, mathematics and physics.
...
In spite the fact that all the above mention difficulties can be also considered as prerequisites in order for one to use SuperCollider, when one posses a laptop and/or PC and is willing to overcome partially some of these difficulties e.g. one's elementary level of English, a new whole world of potentials is unlocked. The above mentioned problematics can be also considered as difficulties of our nowadays society. In order for one to be up to date with the existing trends in music and education one should strongly consider overcoming these above-stated problems.
...
We discussed to include SuperCollider in Music Theory lessons in order to explain the aspects of sound and harmonics. We also consider to teach SuperColider seperately as an individual music course in the following academic years. This will be depedented on the general demand of this course in the above-mentioned music institutions.
...
This proposed method will benefit music teachers as well as music students and thus will evolve our music perseption and what music can be. Needless
